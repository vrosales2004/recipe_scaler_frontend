  ...
  try {
    ...
    if (selectedScaledRecipe) {
      ...
      try {
        ...
        if (response.tipIds && response.tipIds.length > 0) {
          ...
          for (const tipId of response.tipIds) {
            try {
              ...
              // Create a new manual tip with the AI content, attributed to the current user
              const aiContent = `[AI] ${tipData.text}`
              
              // Save as a manual tip with the current user as the author
              const manualTipResponse = await scalingTipsApi.addManualScalingTip({
              ...
              console.log('AI tip saved as manual tip:', manualTipResponse)
              
              const aiTip: ScalingTip = {
                tipId: manualTipResponse.tipId || tipData._id,
                cookingMethod: tipData.cookingMethod,
              ...
              console.warn('Failed to fetch or save AI tip:', tipId, fetchError)
              // Create a fallback tip if fetching fails
              const fallbackContent = `[AI] AI-generated tip for ${newTip.cookingMethod} when scaling ${scalingDirection} for ${baseRecipe.name} (${baseRecipe.originalServings} → ${selectedScaledRecipe.targetServings} servings). This tip was generated by AI based on the specific recipe context.`
              
              try {
                // Try to save the fallback tip as a manual tip
                const fallbackResponse = await scalingTipsApi.addManualScalingTip({
                  cookingMethod: newTip.cookingMethod,
                  direction: scalingDirection,
                  tipText: fallbackContent,
                  addedBy: currentUsername
                })
                
                const fallbackTip: ScalingTip = {
                  tipId: fallbackResponse.tipId || tipId,
                  cookingMethod: newTip.cookingMethod,
                ...
                console.warn('Failed to save fallback tip to backend:', saveError)
                // Create local-only fallback
                const fallbackTip: ScalingTip = {
                  tipId: tipId,
                  cookingMethod: newTip.cookingMethod,
                  direction: scalingDirection as 'up' | 'down',
                ...
                if (!uniqueTips.has(contentKey)) {
                  uniqueTips.set(contentKey, fallbackTip)
                  console.log('Local fallback tip added to unique collection:', fallbackTip)
                }
              }
            }
          }
        ...
        const fallbackContent = `[AI] AI-generated tip for ${newTip.cookingMethod} when scaling ${scalingDirection} for ${baseRecipe.name} (${baseRecipe.originalServings} → ${selectedScaledRecipe.targetServings} servings). ${scalingAdvice}`
        
        try {
          // Try to save the fallback tip to the backend
          const fallbackResponse = await scalingTipsApi.addManualScalingTip({
            cookingMethod: newTip.cookingMethod,
            direction: scalingDirection,
            tipText: fallbackContent,
          ...
          const fallbackTip: ScalingTip = {
            tipId: fallbackResponse.tipId || `ai-fallback-${Date.now()}`,
            cookingMethod: newTip.cookingMethod,
            direction: scalingDirection as 'up' | 'down',
            content: fallbackContent,
            addedBy: currentUsername,
          ...
          console.log('Fallback AI tip saved to backend and added to store:', fallbackTip)
        } catch (saveError) {
          console.warn('Failed to save fallback tip to backend:', saveError)
          // Create local-only fallback
          const fallbackTip: ScalingTip = {
          ...
          console.log('Fallback AI tip added to local store only:', fallbackTip)
        }
      }
    } else {
